...
PKG_VERSION="58978157159418ffcde335e1b17a176a32ccea9a"
PKG_GIT_CLONE_SINGLE="yes"
PKG_GIT_CLONE_DEPTH="1"

PKG_SITE="file://${OLDPWD}/extpackage/u-boot-rppocket"
PKG_URL="${PKG_SITE}"
...
make_target() {
  ...
  if [ -z "${UBOOT_SYSTEM}" ]; then
    ...
    echo "DEBUG=${PKG_DEBUG} CROSS_COMPILE=\"${TARGET_KERNEL_PREFIX}\" LDFLAGS=\"\" ARCH=arm make mrproper"

    	DEBUG=${PKG_DEBUG} CROSS_COMPILE="${TARGET_KERNEL_PREFIX}" LDFLAGS="" ARCH=arm make mrproper
    	DEBUG=${PKG_DEBUG} CROSS_COMPILE="${TARGET_KERNEL_PREFIX}" LDFLAGS="" ARCH=arm make $(${ROOT}/${SCRIPTS}/uboot_helper ${PROJECT} RG351MP ${UBOOT_SYSTEM} config)
    ...
    echo "DEBUG=${PKG_DEBUG} CROSS_COMPILE=\"${TARGET_KERNEL_PREFIX}\" LDFLAGS=\"\" ARCH=arm make \$(${ROOT}/${SCRIPTS}/uboot_helper ${PROJECT} ${DEVICE} ${UBOOT_SYSTEM} config)"
		echo "DEBUG=${PKG_DEBUG} CROSS_COMPILE=\"${TARGET_KERNEL_PREFIX}\" LDFLAGS=\"\" ARCH=arm _python_sysroot=\"${TOOLCHAIN}\" _python_prefix=/ _python_exec_prefix=/ make HOSTCC=\"${HOST_CC}\" HOSTLDFLAGS=\"-L${TOOLCHAIN}/lib\" HOSTSTRIP=\"true\" CONFIG_MKIMAGE_DTC_PATH=\"scripts/dtc/dtc\""
  fi
...
makeinstall_target() {
    ...
    if [ "${DEVICE}" == "RG351P" ]; then
      ...
      cp -f ${PKG_BUILD}/arch/arm/dts/rppocket-uboot.dtb ${INSTALL}/usr/share/bootloader
    fi
